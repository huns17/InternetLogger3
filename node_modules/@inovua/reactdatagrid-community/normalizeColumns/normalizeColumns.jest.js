...
describe('normalizeColumns', function () {
  it('should work ok for most simple case', function () {
    var columns = [{
      name: 'a',
      width: 200
    }, {
  ...
  it('should work when there are columns locked start', function () {
    var columns = [{
      name: 'a',
      width: 200,
      defaultLocked: true
    ...
    expect(result.visibleColumns.map(function (c) {
      return {
        name: c.name,
    ...
    expect(result.lockedStartColumns.map(function (c) {
      return c.name;
    })).toEqual(['a']);
    expect(result.unlockedColumns.map(function (c) {
      return c.name;
    })).toEqual(['b', 'c', 'd']);
    expect(result.visibleColumns.map(function (c) {
      return c.computedWidth;
    })).toEqual([200, 300, 200, 400]);
